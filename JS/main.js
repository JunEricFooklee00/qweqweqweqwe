...
app.get("/Schedule", authenticateToken, (req,res) => {
    ...
    else if(req.user.user === "Employee") res.render("Locals/EmployeeInterfaces/EmployeeCalendar")
...
// app.post("/Signupexample", upload.fields([
//     { name: "images", maxCounts: 1},
//     { name: "image", maxCounts: 1}]), authenticateToken, async (req, res) => {
//     const user = req.user.user;
//     const userId = req.user._id;
//     console.log(user);
//     const hashedPassword = await bcrypt.hash(req.body.password, 10);
...
//             resume:req.files["image"][0].path,
//         }
//         const data2 = await uploadSingleImage(images);
